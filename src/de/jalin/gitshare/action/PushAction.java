...
public class PushAction extends AbstractGitAction implements GitAction {

	final private GitShare git;

	public PushAction(GitShare gitShare) {
		git = gitShare;
	}
	...
	public void performAction() throws GitShareException {
		try {
			git.getGit().push().setCredentialsProvider(git.getCredentialsProvider()).call();
			success();
		} catch (Exception e) {
			failure(e);
