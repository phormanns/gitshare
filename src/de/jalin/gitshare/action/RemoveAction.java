...
public class RemoveAction extends AbstractGitAction implements GitAction {

	final private String path;
	final private GitShare git;

	public RemoveAction(GitShare gitShare, String pathString) {
		path = pathString;
		git = gitShare;
	}
	...
	public void performAction() throws GitShareException {
		try {
			git.getGit().rm().addFilepattern(path).call();
			success();
		} catch (Exception e) {
			failure(e);
